...
matches the number of locks on the door when you're ready to go in (for example, if there's 5 passcodes, 
...
The answer fits within a signed 32-bit integer. Some of the lists are purposely generated without any 
...
def solution(l):
    ...
    while index >= 2:
        index -= 1
        curr = l[index]
        num_factors = len([item for item in l[:index] if curr % item == 0])
        num_multiples = len([item for item in l[index + 1:] if item % curr == 0])
        count += num_factors * num_multiples
    return count
